...
from .utils import DATABASE, get_cols, logging
...
def main(_, name, level, row):
    ...
    for l in range(level, -1, -1):
        sheet = list(filter(lambda x: x.endswith(str(l)), sheets.sheet_names))[0]
        df = sheets.parse(
            sheet_name=sheet, keep_default_na=False, na_values=["", "#N/A"], dtype=str
        )
        ...
        df = df[cols]
        df.to_sql(
            f"{name}_adm{l}_attr", con, if_exists="replace", index=False, method="multi"
        )
        shp_dict = {k: get_cols()[k] for k in cols}
        df = df.rename(columns=shp_dict)
        df.to_sql(
            f"{name}_adm{l}_attr_shp",
            con,
            if_exists="replace",
            index=False,
            method="multi",
