...
def main(level):
    ...
    if table_exists(cur, f'adm{level}_attributes'):
        cur.execute(SQL(query_1).format(
            table_in=Identifier('admx_polygons'),
            id=Identifier(f'adm{level}_id'),
            ids=SQL(',').join(map(Identifier, ids)),
            table_out=Identifier(f'adm{level}_polygons_tmp1'),
        ))
        cur.execute(SQL(queries[level]).format(
            table_in=Identifier(f'adm{level}_polygons_tmp1'),
            table_out=Identifier(f'adm{level}_polygons'),
        ))
        cur.execute(SQL(drop_tmp).format(
            table_tmp1=Identifier(f'adm{level}_polygons_tmp1'),
        ))
        con.commit()
    cur.close()
    con.close()
